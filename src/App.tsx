...
/**
 * StreamZone – Tienda de Streaming (React + TS + Tailwind)
 * Versión local (sin Supabase) + fix de overflow en mobile.
 */
...
function Modal({ open, onClose, children, title, isDark }:{
  open:boolean; onClose:()=>void; children:React.ReactNode; title:string; isDark:boolean;
}){
  if(!open) return null;
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}>
        ...
        <div className="p-3 h-80 overflow-y-auto flex flex-col gap-2">
          {messages.map((m,i)=>(
            <div key={i} className={m.role==='bot'?'self-start':'self-end'}>
              <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${m.role==='bot'? tv(isDark,'bg-zinc-100','bg-zinc-800'): tv(isDark,'bg-zinc-900 text-white','bg-white text-zinc-900')}`}>{m.text}</div>
            </div>
          ))}
        </div>
        <div className="p-3 flex gap-2">
...
export default function App(){
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden ${tv(isDark,'bg-zinc-50 text-zinc-900','bg-zinc-950 text-zinc-100')}`}>
      {/* Navbar */}
      <header className={`sticky top-0 z-30 w-full border-b backdrop-blur ${tv(isDark,'bg-white/80 border-zinc-200','bg-zinc-950/70 border-zinc-800')}`}>
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <Logo className="h-9 w-9 shrink-0" />
            <div className="font-semibold shrink-0">StreamZone</div>
            <div className="hidden sm:block"><Badge isDark={isDark}>Seguridad y confianza</Badge></div>
          </div>

          {/* NAV con scroll horizontal en mobile */}
          <nav className="max-w-full overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            <div className="flex items-center gap-2">
              <button onClick={()=>setView('home')} className={`${view==='home'? tv(isDark,'bg-zinc-900 text-white','bg-white text-zinc-900') : tv(isDark,'bg-zinc-100','bg-zinc-800')} rounded-xl px-3 py-1.5 text-sm whitespace-nowrap`}>Inicio</button>
              <button onClick={goPurchases} className={`${view==='purchases'? tv(isDark,'bg-zinc-900 text-white','bg-white text-zinc-900') : tv(isDark,'bg-zinc-100','bg-zinc-800')} rounded-xl px-3 py-1.5 text-sm whitespace-nowrap`}>Mis compras</button>
              <button onClick={goAdmin} className={`${view==='admin'||view==='adminLogin'? tv(isDark,'bg-zinc-900 text-white','bg-white text-zinc-900') : tv(isDark,'bg-zinc-100','bg-zinc-800')} rounded-xl px-3 py-1.5 text-sm whitespace-nowrap`}>Admin</button>
              {user? (
                <button onClick={logoutUser} className={tv(isDark,'rounded-xl bg-zinc-100 px-3 py-1.5 text-sm','rounded-xl bg-zinc-800 px-3 py-1.5 text-sm')}>Cerrar sesión</button>
              ) : (
                <button onClick={()=>setView('register')} className={tv(isDark,'rounded-xl bg-zinc-100 px-3 py-1.5 text-sm','rounded-xl bg-zinc-800 px-3 py-1.5 text-sm')}>Registrarme</button>
              )}
              {adminLogged && <button onClick={()=>setMenuOpen(true)} className={tv(isDark,'rounded-xl bg-zinc-900 text-white px-3 py-1.5 text-sm','rounded-xl bg-white text-zinc-900 px-3 py-1.5 text-sm')}>Menú</button>}
            </div>
          </nav>
        </div>
      </header>
...
// ========= Subcomponentes =========
function AdminPurchases({ isDark, purchases, setPurchases, onBack, exportCSV }:{
  isDark:boolean; purchases:any[]; setPurchases:React.Dispatch<React.SetStateAction<any[]>>; onBack:()=>void; exportCSV:()=>void;
}){
  const [pFilter, setPFilter] = useState<'pending'|'validated'|'all'>('pending');
  const [query, setQuery] = useState('');
  const countPending = purchases.filter(p=>!p.validated).length;
...
function PurchaseCard({ item, isDark, onToggleValidate, onDelete }:{
  item:any; isDark:boolean; onToggleValidate:()=>void; onDelete:()=>void;
}){
  const days = daysBetween(new Date().toISOString().slice(0,10), item.end);
  const status = days < 0 ? 'Vencido' : days === 0 ? 'Vence hoy' : `${days} dias`;
  return (
...
function AdminDrawer({ open, onClose, isDark, adminEmails, setAdminEmails }:{
  open:boolean; onClose:()=>void; isDark:boolean; adminEmails:string[]; setAdminEmails:(v:string[])=>void;
}){
  const [newEmail, setNewEmail] = useState("");
  const add = ()=>{ const e=newEmail.trim().toLowerCase(); if(!emailOk(e)) return; if(adminEmails.includes(e)) return; setAdminEmails([...adminEmails, e]); setNewEmail(""); };
  const remove = (e:string)=> setAdminEmails(adminEmails.filter(x=>x!==e));
...
function AdminLoginForm({ isDark, onLogin, adminEmails }:{
  isDark:boolean; onLogin:(ok:boolean)=>void; adminEmails:string[];
}){
  const[email,setEmail]=useState(""); 
  const[pass,setPass]=useState('');
  const[msg,setMsg]=useState('');
...
function ReserveForm({ service, onClose, onAddPurchase, isDark, user }:{
  service:any; onClose:()=>void; onAddPurchase:(p:any)=>void; isDark:boolean; user:any;
}){
  const isAnnual=service.billing==='annual';
  const[name,setName]=useState(user?.name||''); const[phone,setPhone]=useState(user?.phone||''); const[start,setStart]=useState(new Date().toISOString().slice(0,10)); const[years,setYears]=useState(1); const[months,setMonths]=useState(1); const[notes,setNotes]=useState('');
  const end=useMemo(()=>{ const d=new Date(start); const add=isAnnual? 12*Number(years): Number(months); d.setMonth(d.getMonth()+add); return d.toISOString().slice(0,10);},[start,months,years,isAnnual]);
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          {isAnnual?(
            <select className={`w-full rounded-xl border px-3 py-2 ${tv(isDark,'border-zinc-300','border-zinc-700 bg-zinc-800 text-zinc-100')}`} value={years} onChange={e=>setYears(Number(e.target.value))}>{[1,2,3].map(y=> <option key={y} value={y}>{y}</option>)}</select>
          ):(
            <select className={`w-full rounded-xl border px-3 py-2 ${tv(isDark,'border-zinc-300','border-zinc-700 bg-zinc-800 text-zinc-100')}`} value={months} onChange={e=>setMonths(Number(e.target.value))}>{[1,2,3,6,12].map(m=> <option key={m} value={m}>{m}</option>)}</select>
          )}
        </div>
        <div><label className={tv(isDark,'text-xs text-zinc-600','text-xs text-zinc-300')}>Fin</label><input disabled className={`w-full rounded-xl border px-3 py-2 ${tv(isDark,'bg-zinc-50 border-zinc-300','border-zinc-700 bg-zinc-800 text-zinc-100')}`} value={end}/></div>
      </div>
