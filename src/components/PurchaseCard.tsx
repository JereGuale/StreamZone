    ...
    <div className={`group relative rounded-3xl border transition-all duration-500 overflow-hidden ${tv(isDark, 'bg-white border-gray-200 shadow-xl', 'bg-[#0B1120] border-white/5')}`}>
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <div className="flex items-center justify-end sm:justify-start gap-2">
            ...
            {showAdminActions && (
              <div className={`flex px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg font-bold tracking-widest uppercase items-center gap-1 sm:gap-1.5 text-[8px] sm:text-[9px] shadow-sm border whitespace-nowrap ${days < 0
      ...
      {isOpen && (
        <div className={`p-4 sm:p-5 transition-all duration-500 border-t flex flex-col gap-4 ${tv(isDark, 'bg-gray-50/50 border-gray-100', 'bg-[#050914] border-white/5')}`}>
          ...
          {showAdminActions && (
            <div className="mt-4 grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-start gap-2">
              <button
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleValidate(); }}
                className={`flex items-center justify-center gap-1.5 px-3 sm:px-5 py-2.5 rounded-xl text-white text-[10px] sm:text-xs font-black transition-all shadow-lg active:scale-95 whitespace-nowrap ${days < 0
                  ? 'bg-red-600 hover:bg-red-700 shadow-red-600/20'
                  : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/20'}`}
              >
                <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                {days < 0 ? 'Vencida' : 'Activa'}
              </button>
              ...
              <button
                onClick={onEdit}
                className={`flex items-center justify-center gap-1.5 px-3 sm:px-5 py-2.5 rounded-xl border text-[10px] sm:text-xs font-black transition-all active:scale-95 whitespace-nowrap ${tv(isDark,
                  'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300 shadow-sm',
                  'bg-white/5 border-white/10 text-zinc-100 hover:bg-white/10 hover:border-white/20'
                )}`}
              >
                <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              ...
              >
                <svg className={`w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 ${tv(isDark, 'text-emerald-500', 'text-emerald-400')}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
